...
import it.csi.dma.dmafonti.exception.RESTException;
...
public abstract class RESTBaseService {
	...
	protected Response handleException(Exception e) {
		...
		res = ErroreBuilder.from(Status.INTERNAL_SERVER_ERROR).title(msg).dettaglio("ERR_999", e.getMessage()).exception().getResponse();
